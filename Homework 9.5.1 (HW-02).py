...
# Здесь просто задаем цвета. Они не соответствуют своим названиям, но главное всё сгруппировано в одном месте и
# при желании цвета можно легко поменять не меняя всей логики приложения
class Color:
    yellow2 = '\033[1;35m'
    reset = '\033[0m'
    blue = '\033[0;34m'
...
class Field(object):
    ...
    # Рисуем поле. Здесь отрисовка делится на две части. Так как отрисовка весов клеток идёт по другому
    ...
    def ships_setup(self, player):
        ...
        for ship_size in Game.ships_rules:
            ...
            while True:
                ...
                # Сюда мы добираемся только если корабль не поместился. Пишем пользователю, что позиция неправильная
...
class Player(object):
    ...
    def make_shot(self, target_player):
        ...
        # промазал, попал или убил (в случае если убил, возвращается корабль)
...
if __name__ == '__main__':
    ...
    # создаем саму игру в бесконечном цикле
    game = Game()
